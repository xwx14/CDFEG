...
| 多单元类型同场 → post 写同名 Result 多段、parser 后者覆盖 | 当前 4 示例算例均单单元类型，无影响；标注为已知限制（见 §7） |
...
2. **多单元类型同场同名 Result 覆盖**：`post` 对每单元类型写一个同名 `Result` 段，parser 以 `(name,step)` 为 key 后者覆盖前者。当前算例均单单元类型无影响；未来混用需按单元类型拆 result_name 或扩展 parser。
