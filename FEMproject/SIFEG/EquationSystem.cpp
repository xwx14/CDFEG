...
int SIFEG::EquationSystem::init(std::vector<std::set<int>>& mht)
{
	int nEq = mht.size();
	_f.resize(nEq);
	_colMap.resize(nEq);
...
Eigen::SparseMatrix<double, Eigen::RowMajor>& SIFEG::EquationSystem::convert2Eigen()
{
...
int SIFEG::EquationSystem::solve()
{
	int n = _f.size();
	convert2Eigen();
...
void SIFEG::EquationSystem::adda(const std::vector <double>& estifn, const std::vector<int>& equIds)
{
	int n0, n1;
	int nd = equIds.size();
	for (int i = 0; i < nd; ++i)
...
void SIFEG::EquationSystem::addFirstBC(int equId, double val)
{
	// 检查equId是否有效
...
void SIFEG::EquationSystem::addSecondBC(int equId, double val)
{
	// 检查equId是否有效
...
void SIFEG::EquationSystem::applyFirstBCs(const std::map<int, double>& bc1s, const std::vector<int>& ida)
{
...
void SIFEG::EquationSystem::applySecondBCs(const std::map<int, double>& bc2s, const std::vector<int>& ida)
{
	// 批量应用第二类边界条件
	for (auto& bc : bc2s)
	{
...
int SIFEG::EquationSystem::calRightVals()
{
