...
- **`test/test*.py` 是代码生成器，运行会覆盖 `FEMproject/sample/` 下对应示例的 C++ 源码**。各示例遵循"pyTool 生成框架 + 人工填充逻辑"模式：DEl2D 已对齐命名（`testDEL2D.py` 生成 `del2dData`/`DelDispFieldData`/`DelQ4g` 框架，Newmark 动力学逻辑手填），可重生成；重生成前需保留手填的 `run`/`uEle`/`uPhy` 体。
